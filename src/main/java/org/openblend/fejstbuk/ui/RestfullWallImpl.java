...
public class RestfullWallImpl implements RestfullWall {
    ...
    public List<Owned> activity() throws IOException {
        ...
        Query query = em.createQuery("select from Post p where p.user in (:users) order by p.timestamp desc");
        query.setMaxResults(Wall.SIZE);
        query.setParameter("users", users);
        list.addAll(query.getResultList());
        // user and his friend's likes
        query = em.createQuery("select from Like l where l.user in (:users) order by l.timestamp desc");
        query.setMaxResults(Wall.SIZE);
        query.setParameter("users", users);
        list.addAll(query.getResultList());
